...
# Description              : Data loading utilities for federated learning.
...
from clasification_model import (
    build_input_tensor,
...
def load_federated_clients(clean_base_path):
    """
    Load all federated clients' data from client subfolders.
...
def summarize_clients(clients_data):
    """
    Print a summary for each client.
...
def inspect_labels(clients_data):
    """
    Inspect label id / label name consistency across clients.
...
def get_client_sheep(df_client):
    """
    Get sorted list of unique sheep identifiers for a client.
    """
...
def choose_validation_sheep(all_sheep, test_sheep):
    """
    Choose a validation sheep different from the test sheep.
    """
...
def split_client_by_sheep(df_client, test_sheep, val_sheep):
    """
    Split one client's DataFrame into train/val/test by sheep.
    """
...
def prepare_client_splits(clients_data, random_seed=42):
    """
    Build train/val/test splits for each client using sheep-based splitting.
    """
...
def summarize_client_splits(client_splits):
    """
    Print train/val/test split summary for each client.
    """
    print("\n" + "=" * 70)
...
def prepare_client_tensors(client_splits):
    """
    Convert each client's train/val/test DataFrames into normalized tensors.
    """
    client_tensors = {}
...
def summarize_client_tensors(client_tensors):
    """
    Print tensor shape summary for each client.
    """
    print("\n" + "=" * 70)
