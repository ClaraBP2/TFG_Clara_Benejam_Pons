...
# Description              : Local baseline training + confusion matrices for federated vs local comparison.
# Usage                    : Call run_evaluation() after run_fedavg() in run.py
...
def run_local_baselines(client_tensors, local_epochs=50, results_dir="results/evaluation"):
    """
    Function: Train one independent model per client (no federation) and evaluate on test set.
    Args:
...
def plot_confusion_matrix(ax, cm, title, class_names):
    """
    Function: Plot a single normalized confusion matrix on a given axes.
    Args:
    ...
    Returns:
        im: The image object for the confusion matrix.
    """
...
def plot_all_confusion_matrices(global_model, baseline_models, client_tensors, finetuned_models, results_dir="results/evaluation"):
    """
    Function: Plot confusion matrices for federated global model, fine-tuned model, and local baseline, one row per client.
    Args:
...
def build_comparison_table(federated_results_df, finetuned_results_df, baseline_results_df, results_dir="results/evaluation"):
    """
    Build a comparison table of test metrics for federated, fine-tuned, and local baseline models.
    Args:
        federated_results_df (pd.DataFrame): Test metrics from federated training.      
        finetuned_results_df (pd.DataFrame): Test metrics from fine-tuning.
        baseline_results_df (pd.DataFrame): Test metrics from local baseline training.
        results_dir (str): Directory to save the comparison CSV.
    Returns:
        comp (pd.DataFrame): Comparison table with deltas.
    """
    os.makedirs(results_dir, exist_ok=True)
...
def plot_comparison_bars(comparison_df, results_dir="results/evaluation"):
    """
    Function: Plot bar charts comparing federated vs local baseline metrics per client.
    Args:
        comparison_df (pd.DataFrame): Output of build_comparison_table().
        results_dir (str): Directory to save the comparison bar chart.
    Return: None
    """
    os.makedirs(results_dir, exist_ok=True)
...
def run_evaluation(global_model, final_results_df, client_tensors,
    ...
    """
    Function: Full evaluation pipeline:
    ...
    Args: 
        global_model: Trained federated global Keras model.
        final_results_df (pd.DataFrame): Output of run_fedavg() with test metrics per client.
        client_tensors (dict): Output of prepare_client_tensors.
        local_epochs (int): Max epochs for local baseline training.
        finetune_epochs (int): Max epochs for fine-tuning the global model per client.
        results_dir (str): Directory to save results.
    Returns: 
        tuple: (baseline_df, finetuned_df)
    """
    os.makedirs(results_dir, exist_ok=True)
...
def finetune_global_model(global_model, client_data, finetune_epochs=5):
    """
    Function: Fine-tuning local del modelo global para personalización.
    Args:
        global_model: Trained federated global Keras model.
        client_data (dict): Local tensors (X_train, y_train, X_val, y_val, X_test, y_test).
        finetune_epochs (int): Max epochs for fine-tuning.
    Returns:
        local_model: Fine-tuned Keras model for the client.
    """
