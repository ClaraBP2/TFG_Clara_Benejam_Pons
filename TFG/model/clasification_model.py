...
def build_input_tensor(df):
    ...
    # Convert labels to int and ensure they're valid
    y = df["label"].astype(int).to_numpy(dtype=np.int32)

    # Remove rows with invalid labels (should be 0, 1, 2)
    valid_mask = (y >= 0) & (y < 3)
    X = X[valid_mask]
    y = y[valid_mask]
...
def build_model(input_shape=(300, 4, 1), num_classes=3):
    model = Sequential([
        ...
        Conv2D(16, kernel_size=(5, 2), activation="relu"),
...
def evaluate_fold(df_study, test_sheep, val_sheep):
    ...
    # Class weights.
    # The results are better with this hardcoded weights, as they were tuned for this specific dataset and task.
    class_weight = {
    0: 2.0,   # active
    1: 1.2,   # grazing
    2: 0.8    # inactive
